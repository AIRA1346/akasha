...
> **구현 단계:** Phase 0–1 완료 후 Phase 2 런타임 전환
...
- bundle-only 첫 release에서는 사용자 Vault가 아니라 registry disk cache와 legacy
  registry cache만 한 번 무효화한다. 실제 migration은 Phase 2에서 구현한다.

## Phase boundary

Phase 0–1은 baseline, strict gate, 결정적 full-bundle builder, release/CI 검증을
구현한다. 현재 production runtime의 CDN 호출, sync UI, cache 우선순위는 이 단계에서
변경하지 않는다. `RegistrySource` 도입, production remote 호출 제거, registry cache
migration은 Phase 2에서 수행한다.
...
전체 작품 검색과 상세 열기는 Phase 2 완료 후 네트워크 없이 동작할 수 있다. 패키지와

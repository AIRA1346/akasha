...
| **External ID** | **`externalIds.wikidata` (spine)** · `tmdb` · `mal` · `steam` … | 중복 탐지·대조용 (자동 merge 근거 단독 사용 금지) |
| **Graph (내부)** | `wikidataRelations[]` · `extensions.seasons[]` | Wikidata P/Q 참조만 — UX용 시즌 `wk_` 남발 금지 |
...
| **시즌/파트** | 애니 1기 vs 2기 | 기본 **동일 `wk_`** + `extensions.seasons[].wikidata` (별도 workId 남발 금지) |
...
1. `externalIds.wikidata` exact match (spine 우선) · 기타 `externalIds` exact match

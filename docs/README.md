...
인지 부하를 최소화하고 문서의 신뢰도를 유지하기 위해, 모든 문서는 다음 카테고리로 엄격히 분류하여 운영합니다.
...
| [RELATION_TIERS_AND_ASSERTIONS_ADR.md](active/RELATION_TIERS_AND_ASSERTIONS_ADR.md) | **Relation Tiers ADR** | 링크·Canvas 연결·독립 관계 주장을 구분하는 의미 계약. |
| [LIFECYCLE_TOMBSTONE_SUPERSESSION_ADR.md](active/LIFECYCLE_TOMBSTONE_SUPERSESSION_ADR.md) | **Lifecycle ADR** | 물리 삭제와 철회·대체·병합·tombstone을 구분하는 의미 계약. |
| [EXTENSION_NAMESPACE_AND_RESERVED_FIELDS_ADR.md](active/EXTENSION_NAMESPACE_AND_RESERVED_FIELDS_ADR.md) | **Extension Namespace ADR** | 미래 AKASHA 필드와 사용자·도구 YAML의 소유권·충돌 규칙. |
| [GATEWAY_PERMISSION_AND_RECEIPT_ADR.md](active/GATEWAY_PERMISSION_AND_RECEIPT_ADR.md) | **Gateway Authority ADR** | 외부 도구 권한·철회·idempotency 영수증 계약. |

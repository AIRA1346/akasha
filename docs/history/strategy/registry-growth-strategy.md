...
>
> **운영 (2026-06-10):** [catalog-growth-charter.md](../programs/catalog-growth-charter.md) — SD2.6 hold **해제** · insert와 아키텍처 검증 **병행**.  
> 커뮤니티·사용자 등록: [contribution-model-strategy.md](../policy/contribution-model-strategy.md).
...
| **데이터** | 엄선 430 · Fact-only Tier 1 | **수백만 stub·dedupe** — **미해결 과제** |
| **병목 (오늘)** | 작품 **수** + search_index at G1 | **동시 해결** |
| **병목 (100k+)** | 메타 **품질·다국어·franchise** + 검색 | 저장은 2순위 이후 |

```
[병행]  작품 추가 + 아키텍처 검증 (dedupe · search_index · gate)  ← catalog-growth-charter
[지속]  넣은 작품을 찾을 수 있는가?  ← SW1
[지속]  넣은 작품이 올바른가?      ← URV
[알려진] 넣은 작품을 빨리 읽을 수 있는가? ← search_index (증분·재측정)
```

**430작 Steam 출시는 목표에서 제외** — G1(~5k) 밴드로 **검색 가치**를 맞춘 뒤 출시.

---

## 2. 성장 경로 (430 → 5,000,000)

### 2.1 단계 요약

| 단계 | 규모 | 시기 (목표) | Registry 성격 | 주 병목 |
|------|------|-------------|---------------|---------|
| **G0** | **430→G1** | 2026 Steam v1 **전** | Fact-only · **확장 중** | **커버리지** + search_index |
...
| 항목 | 방식 |
|------|------|
| 수집 | 수동 큐레이션 · AniList bulk **금지** · Tier 1 **Fact only** (poster·description ❌) |
...
| Discovery | Wikidata manga + Open Library (Signal) — AniList API **금지** ([discovery-source-decision.md]](../discovery-source-decision.md)) |
...
### 6.1 세 축 비교 (430 · 50k · 500k · 5M)

| 축 | 430 (오늘) | 50k | 500k | 5M |
|----|------------|-----|------|-----|
...
| 순위 | 병목 | 근거 |
|------|------|------|
| **1 (G0~G1)** | **데이터 확보 (커버리지)** | 430는 구조 검증용 · 작품 수 자체가 제품 가치 상한 |

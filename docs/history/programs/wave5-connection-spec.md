...
Record (.md body)
    │
    ├─ [[pe_u_xxx|나츠키 스바루]]     → explicitId (R2-B canonical · picker)
    ├─ [[wk_000000001|에이티식스]]     → explicitId
    ├─ [[pe_u_abcd1234]]              → explicitId (label 없음 · authoring 비권장)
    └─ [[Tiger]]                      → titleOnly (fallback · resolve via catalog)
```

**Link Identity SSOT:** [link-identity-policy.md](../policy/link-identity-policy.md) — R2-B Step 0 Locked.

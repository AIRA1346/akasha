...
| 항목 | UX-1 이전 | 현재 결과 |
|---|---|---|
| Preset | legacy ID 4개 | canonical ID 5개 + `LibraryTheme` adapter |
...
| Nocturne | Steam Inventory POC에만 존재 | canonical neutral fallback preset 등록, 최종 art·SKU 후속 |

검증: root analyze 0 · root test 974 · Windows debug build PASS. Production IAP와 premium 구매 UI는 계속 비활성이다.

Style 1차 이관으로 전체 `lib` 기준 `AkashaColors.*` 직접 참조는 444→424, `context.akashaPalette` 사용은 86→94로 바뀌었다. UI scope 기준 static color 후보는 399→379다. 나머지는 아래 Phase별 inventory에 따라 계속 줄인다.
...
|---|---|---|---|---|---|
| `classicDark` | bundled/free | near-black · deep navy · blue/violet accent | 우주 궤도, 성좌, 지식 연결 | 절제된 violet/blue glow, 희미한 별빛 | 실제 palette 이관 · Astral reference 방향 · 안전한 fallback |
| `midnightBlue` | bundled/free | deep navy · cool blue accent | 최종 전용 art 미정 | 낮은 blue glow, 차분한 깊이 | 실제 palette 이관 · solid/gradient fallback |
| `sakura` | premium | charcoal rose · warm pink accent | 벚꽃 가지와 꽃잎 | warm pink glow, 선택적 petal ambient | ID와 fallback preset만 · 최종 art 후속 |
| `amethyst` | premium | black purple · amethyst accent | 자수정 crystal | purple bloom/glow | ID와 fallback preset만 · 최종 art 후속 |
| `nocturne` | premium | **미정 — 임의 결정 금지** | **reference 미제공** | **미정** | ID와 중립 fallback preset만 |

Nocturne의 이름·premium 분류만 확정됐다. palette, art, effect는 별도 reference 또는 사용자 결정 전까지 Classic Dark geometry를 사용하는 중립 fallback으로 둔다. Steam POC의 ItemDef와 비용은 시각 방향을 결정하지 않는다.
...
| `AkashaColors.*` 직접 참조 | 399 | 113 | Classic 고정색이므로 가장 큰 theme 누수 |
...
| 파일 | 현재 사용 | 목표 semantic token/component | Phase | 위험 |
|---|---|---|:---:|:---:|
| `lib/screens/home/home_browse_search_chrome.dart` | 고정 accent, geometry, radius | Search chrome + accent/button tokens | UX-2 | Critical |
| `lib/widgets/dashboard_sidebar.dart` | Sidebar `280px`, 일부 고정 accent | `AkashaShellMetrics.sidebarWide=256` + nav tokens | UX-2 | Critical |
| `lib/widgets/dashboard_sidebar_footer_part.dart` | palette가 아닌 Classic static surface | Sidebar semantic surface | UX-2 | High |
| `lib/screens/home/home_shell_scaffold_bottom_nav_part.dart` | `56px`, `fontSize:10`, static text color | Dock metric + shared destination/nav item | UX-2 | Critical |
| `lib/screens/home/views/dashboard_preview_panel.dart` | Work Preview `320px`, 직접 spacing | 공통 Preview rail `288px` + section tokens | UX-2/4 | Critical |
| `lib/screens/home/views/entity_dashboard_preview_panel.dart` | Entity Preview `320px`, 별도 geometry | 동일 Preview rail contract | UX-2/4 | Critical |

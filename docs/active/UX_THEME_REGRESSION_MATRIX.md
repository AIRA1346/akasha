...
현재 다섯 preset 모두 code fallback을 사용한다. Classic Dark와 Midnight Blue는 무료 배포 가능한 palette가 구현됐고, Sakura·Amethyst·Nocturne의 최종 artwork는 아직 승인·통합되지 않았다.
...
Classic Dark와 Midnight Blue는 Windows의 `960×640` stable component fixture를 screenshot golden으로 추가 고정한다.

- `test/goldens/theme_classic_dark_standard.png`
- `test/goldens/theme_midnight_blue_standard.png`

Golden은 palette·Material component·fallback backdrop의 시각 기준이다. 실제 제품 viewport overflow와 geometry는 위의 3개 viewport widget test가 담당한다.
...
2026-07-14 결과: root analyze **0**, root test **1121**, Windows Debug/Release build PASS.

---

## 5. 남은 UX-5 범위

- **UX-5B:** Classic Dark·Midnight Blue의 승인된 실제 backdrop/Hero/texture asset 통합과 성능·가독성 검토

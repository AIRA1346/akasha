...
2026-07-14 UX-5C 결과: root analyze **0**, root test **1124**, Windows Debug/Release build PASS. 공식 5테마의 backdrop/Hero 10개가 Windows golden에서 실제 decode·paint됐고 Release asset bundle의 SHA-256이 workspace provenance와 모두 일치한다. `AkashaColors.*` 고정 측정은 **328 lines / 81 files**를 유지한다.

2026-07-15 UX-5D 결과: root analyze **0**, root test **1124**, Windows Debug/Release build PASS. 단일 `AkashaThemeRegistry`, ID-only preference migration, Backdrop·Hero·Interaction·Motion effect group을 고정했다. 기존 10개 Windows golden은 갱신 없이 통과했고 `AkashaColors.*`는 **328 lines / 81 files**를 유지한다.

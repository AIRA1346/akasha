...
| **Steam Inventory** | 기존 POC purchase/exchange E2E 통과 ✅ · production ItemDefs private partner 게시와 KRW 가격 조회 통과 · 첫 production `StartPurchase(40110)`은 Overlay checkout 전에 실패 · 현재 SteamPipe 스크립트가 개발용 `steam_appid.txt`를 depot에 포함할 수 있는 확정 packaging defect 발견 · 정확한 phase/`EResult` 진단, Overlay/subscription capability gate, 안전한 depot/library 재검증이 [release readiness](STEAM_SERVICE_RELEASE_READINESS.md)에 남음 · production IAP는 계속 비활성 |

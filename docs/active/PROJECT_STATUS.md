...
| **Steam Inventory** | 기존 POC purchase/exchange·Overlay E2E 통과 ✅ · production ItemDefs private partner 게시와 KRW 가격 조회 통과 · 첫 production `StartPurchase(40110)`은 checkout 전에 실패 · initialized/logged-on/subscribed/Overlay/승인 가격 capability gate와 phase/API handle/`EResult`/order/trans sanitized 진단 복사 구현 · 별도 depot stage가 `steam_appid.txt`/PDB를 제외하고 97-file SHA-256 manifest 생성 · production 재시도와 Steam-library/Steamworks 설정 증거는 [release readiness](STEAM_SERVICE_RELEASE_READINESS.md)에 남음 · production IAP는 계속 비활성 |

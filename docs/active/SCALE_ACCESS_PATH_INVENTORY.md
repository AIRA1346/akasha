...
refreshes the bounded projection without a direct whole-Vault reread; the
add-dialog, registry-archive, and AI Markdown import routes share that bounded
Work persistence path. Other action surfaces and broad legacy-watch reloads
remain separate migration work before the scale gate passes.

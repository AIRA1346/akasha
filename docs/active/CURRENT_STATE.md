...
> - **Steam/build identity hardening** — `ae18be4c` `build(steam): reject personal paths in binary payloads` (binary payload scan · personal/repo path · credential content). Steam release evidence archived at [`AKASHA_Product/release-evidence/steam`](../../AKASHA_Product/release-evidence/steam/README.md) (**REL-EVID-01**: live receipt + pre-upload seal + historical sandbox receipt; byte-verified). `depot_windows` binary stage intentionally not retained. Evidence archive ≠ Commerce CURRENT-RC-PASS ≠ Overall Go. Worktree closure ≠ Steam IAP readiness.

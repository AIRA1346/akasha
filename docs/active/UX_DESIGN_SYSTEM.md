...
현재 no-IAP 런타임 Theme Gallery는 bundled 2종과 premium 예정 3종을 함께 노출한다. legacy 저장 ID는 `AkashaThemeRegistry.persistedAliases`를 거쳐 canonical ID로 이관하며, 알 수 없는 ID는 미래 preset 복구 가능성을 위해 preference에 보존한다. 상세 migration 기록은 [UX_THEME_MIGRATION_INVENTORY.md](UX_THEME_MIGRATION_INVENTORY.md)를 따른다.

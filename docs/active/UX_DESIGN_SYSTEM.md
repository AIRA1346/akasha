...
desktop의 우측 288px는 선택 유무와 무관하게 항상 예약하는 **Inspector rail**이다. 선택이 없으면 현재 목적지, archive summary, vault 상태 같은 기본 모듈을 표시하고, 선택이 생기면 같은 공간의 내용만 contextual preview로 교체한다. 따라서 선택 열기·닫기는 중앙 canvas의 폭과 scroll 위치를 바꾸지 않는다. 새 기본 정보는 `HomeInspectorModule`로 추가하며 Shell geometry나 Work/Entity preview를 수정하지 않는다. compact에서는 이 공간을 예약하지 않고 선택 정보만 sheet로 표시한다.

...
### 4.0 Windows frame과 표시 모드

Windows에서는 앱 콘텐츠만이 아니라 창 프레임도 Shell 계약에 포함한다. custom chrome은 `32px` 고정 높이이며 앱 identity, drag 영역, 최소화·최대화/복원·닫기 조작을 제공한다. 색과 border는 현재 root theme를 따르지만 높이와 hit target은 테마별로 달라지지 않는다.

- 기본 실행은 크기 조절 가능한 창모드다.
- 최대화는 일반적인 넓은 작업 상태다.
- `F11`은 창 chrome만 숨기는 borderless fullscreen을 전환한다. Home의 Sidebar·Top bar·Preview·Dock은 유지한다.
- fullscreen에서 `Esc`는 drawer/Preview보다 먼저 fullscreen을 종료한다.
- Graph·Timeline·Canvas의 패널까지 숨기는 집중 모드는 fullscreen과 구분하며 후속 기능으로 둔다.
- 최소 창 크기는 외곽 창 기준 `1024×720`이고, 전환 후 이전 창 bounds는 OS window manager가 복원한다.
...
| 구간 | Left sidebar | Preview / Inspector | Bottom dock |
|---|---|---|---|
| **Wide** `>= 1440` | `256px` 고정 | `288px` 고정 Inspector rail | 없음 (Sidebar가 주 내비) |
| **Standard** `1320–1439` | `232px` 고정 | `288px` 고정 Inspector rail | 없음 (Sidebar가 주 내비) |
| **Compact** `< 1320` | drawer | selection bottom sheet | 주 내비게이션 |

`standardBreakpoint = 232 + 288 + 800 = 1320`이다. Inspector가 열린 desktop 레이아웃에서 center는 절대 `800px` 미만이 되지 않는다.
...
Sidebar 하단에는 별도의 `접기` 행을 두지 않는다. Sidebar 표시 전환은 Top bar toggle과 `Ctrl+B` 한 계약으로 통일하고, Compact에서는 같은 동작이 drawer를 연다.
...
desktop의 우측 **Inspector rail**은 기본적으로 288px를 예약한다. 선택이 없으면 현재 목적지, archive summary, vault 상태 같은 기본 모듈을 표시하고, 선택이 생기면 같은 공간의 내용만 contextual preview로 교체한다. 따라서 선택 preview의 열기·닫기는 중앙 canvas 폭을 바꾸지 않는다. 사용자가 `Ctrl+N` 또는 AppBar control로 Inspector 자체를 닫을 때만 rail 공간을 해제하고 중앙 canvas를 확장하며, 다시 열면 보존된 선택·scroll 상태를 복원한다. 열림 상태는 다음 실행에도 유지한다. 새 기본 정보는 `HomeInspectorModule`로 추가하며 Shell geometry나 Work/Entity preview를 수정하지 않는다. compact에서는 고정 공간을 예약하지 않고, **선택이 있을 때만** sheet로 표시한다. compact selection sheet는 desktop Inspector preference(`isInspectorOpen`)와 독립이다.

### 4.7 Bottom dock

Dock과 Sidebar는 반드시 하나의 `AppDestination` 모델과 selection SSOT를 공유한다.

- Sidebar는 위치·아카이브 맥락·최근 기록을 제공한다.
- Dock은 compact의 목적지 전환만 제공한다. standard/wide에서는 Sidebar와 중복 렌더링하지 않는다.
...
- 배경·터치 효과는 root `AkashaEffectsHost` 한 곳에서 합성하고 controller의 `Listenable → CustomPainter` repaint만 허용한다. animation frame마다 Home Shell을 rebuild하지 않는다.
- 효과별 활성 요소 수는 `AkashaEffectRuntimePolicy`의 전역 예산과 spec의 상한 중 작은 값으로 제한한다.
...
2026-07-13 UX-1 foundation으로 `AkashaThemePreset`, `ThemeCatalogEntry`, `AkashaThemeController`, `AkashaThemeBackdrop`과 theme harness가 구현됐다. 2026-07-15 UX-5D에서 preset·catalog·persisted alias 등록을 `AkashaThemeRegistry` 하나로 통합하고, 저장 호환은 ID-only `AkashaThemePreferences`로 한정했다. `LibraryTheme` runtime adapter는 제거됐다. 여러 화면의 raw `Color`, `AkashaColors`, 고정 geometry 이관은 아직 후속 Phase에 남아 있다.
...
2026-07-22 확장 기반에서 `AkashaThemeEffects.extensions`는 실행 코드가 아닌 `AkashaEffectSpec` 목록만 가진다. spec은 안정적인 effect ID, background/interaction layer, motion 필요 여부, intensity, 최대 활성 요소만 선언한다. 실제 controller factory는 `AkashaEffectRegistry`의 명시적 allowlist에 등록하며, 알 수 없는 ID는 안전하게 아무것도 렌더링하지 않는다. interaction layer는 앱의 pointer hit test를 막지 않고, 모든 paint layer는 semantics와 input에서 제외한다. 새 효과는 다음 순서를 따른다.

1. bounded controller와 painter를 구현하고 registry에 ID를 등록한다.
2. runtime policy의 element budget과 intensity를 사용한다.
3. reduced motion, high contrast, compact width에서 동작을 검증한다.
4. pointer 전달, child rebuild 부재, controller dispose를 widget test로 고정한다.
...
무료/유료 분류와 승인 가격은 공식 상품 계획이고, 판매 활성 상태와는 별개다. `steamInAppPurchasesEnabled=false` 빌드에서도 Theme Gallery에는 공식 5종을 모두 노출한다. Classic Dark와 Midnight Blue만 적용할 수 있고 premium 3종은 `500 Astra 또는 500 Echo · 출시 예정`으로 발견 가능하게 하되 활성 구매 버튼·가짜 잔액·가짜 잠금 해제 동작은 표시하지 않는다.

현재 no-IAP 런타임 Theme Gallery는 bundled 2종과 premium 예정 3종을 함께 노출한다. legacy 저장 ID는 `AkashaThemeRegistry.persistedAliases`를 거쳐 canonical ID로 이관하며, 알 수 없는 ID는 미래 preset 복구 가능성을 위해 preference에 보존한다. 상세 migration 기록은 [UX_THEME_MIGRATION_INVENTORY.md](../history/closure-2026-07/UX_THEME_MIGRATION_INVENTORY.md)를 따른다.

### 6.3 목표 표현 계층

| 계층 | 책임 | 테마 변경 가능 |
|---|---|:---:|
| `AkashaThemeRegistry` | preset·catalog·legacy alias의 단일 등록 SSOT | 등록 시 |
| `AkashaThemePreset` | canonical id, palette, assets, effects | 예 |
| `ThemeCatalogEntry` | 표시명/l10n, bundled/premium, offer state, 가격, entitlement metadata | 해당 없음 |
| `AkashaPalette` | 배경·표면·텍스트·border·accent·focus·상태 의미색 | 예 |
| `AkashaThemeAssets` | hero, texture, optional ambient artwork, fallback | 예 |
| `AkashaThemeEffects` | Backdrop·Hero·Interaction·Motion 효과 그룹 | 예 |
...
Backdrop의 scrim·texture·ambient 값을 Hero glow·shadow와 분리하고, hover·pressed·focus 및 motion duration을 별도 그룹으로 둔다. 한 surface의 조정이 다른 surface에 암묵적으로 전파돼서는 안 된다. geometry와 hit target은 effect 계약에 넣지 않는다.
...
- offerState: included | planned | purchasable | paused
- astraCost?
- echoCost?
- entitlementItemDefId?
```

`requiresIap`는 visual preset의 필드가 아니다. `offerState`도 사용자별 소유권 상태와 분리한다. premium 테마는 `planned` 상태에서 발견 가능하지만 구매할 수 없고, 실제 SKU·가격·provider가 준비된 뒤에만 `purchasable`이 된다. Sakura와 Amethyst의 production SKU·가격은 아직 없으며 임의로 만들지 않는다. Nocturne의 Steam Inventory POC ItemDef `20001`과 Astra 비용 `100`은 feasibility evidence일 뿐 production catalog 확정값이 아니다.
...
no-IAP 빌드에서도 premium entry는 Theme Gallery에 표시하되 `planned` 상태에서는 선택·적용할 수 없다. 기존 저장값이 premium이면 preferred는 보존하고 effective만 Classic Dark로 안전하게 해석한다. provider가 나중에 `owned`를 확인하면 보존한 preferred를 다시 적용할 수 있어야 한다.
...
- 증거와 Phase별 대상: [UX_THEME_MIGRATION_INVENTORY.md](../history/closure-2026-07/UX_THEME_MIGRATION_INVENTORY.md)
...
검증 기준: 해당 단계의 root analyze/test와 Windows Debug/Release build가 통과했다. 현재 품질 기준선은 [CURRENT_STATE.md](CURRENT_STATE.md)를 따른다.
...
UX-3 검증: 해당 단계의 root analyze/test와 Windows Debug/Release build가 통과했다. 현재 품질 기준선은 [CURRENT_STATE.md](CURRENT_STATE.md)를 따른다.
...
UX-4A 검증: 해당 단계의 root analyze/test와 Windows Debug/Release build가 통과했다.

UX-4B 검증: 해당 단계의 root analyze/test와 Windows Debug/Release build가 통과했다. `AkashaColors.*` 고정 측정 계약은 UX-4B 시작 **354 lines / 90 files**에서 **342 lines / 86 files**로 감소했다.

UX-4C 검증: 해당 단계의 root analyze/test와 Windows Debug/Release build가 통과했다. 3 viewport·125% text·Classic/Midnight context header geometry와 Explore-only entity strip 라우팅을 검증했다. `AkashaColors.*`는 **342 lines / 86 files**에서 **334 lines / 83 files**로 감소했다.

UX-4D 검증: 해당 단계의 root analyze/test와 Windows Debug/Release build가 통과했다. Graph/Timeline context header와 공통 empty state를 3 viewport·125% text·Classic/Midnight에서 검증하고 Graph 탭의 selected/button/tap semantics를 고정했다. Graph·Timeline에서는 browse search, catalog loading, daily recall을 렌더하지 않으며 Timeline은 vault path 변경 시 기존 loader로 다시 읽는다. `AkashaColors.*`는 **334 lines / 83 files**에서 **328 lines / 81 files**로 감소했다.
...
- [x] reduced motion에서 ambient artwork와 decorative motion만 제거하는 공통 resolver
...
- [x] preset·catalog·alias 단일 registry와 Backdrop·Hero·Interaction·Motion effect 계약 — UX-5D
...
UX-5A 검증: 해당 단계의 root analyze/test와 Windows Debug/Release build가 통과했다. `AkashaColors.*` 고정 측정은 UX-4D와 동일한 **328 lines / 81 files**다.

UX-5B 검증: 해당 단계의 root analyze/test와 Windows Debug/Release build가 통과했다. Release asset bundle의 4개 hash가 workspace provenance와 일치하며 `AkashaColors.*`는 **328 lines / 81 files**를 유지한다.

UX-5C 검증: 해당 단계의 root analyze/test와 Windows Debug/Release build가 통과했다. 공식 5테마 backdrop/Hero 10개의 Windows decode·paint golden과 Release bundle hash 일치를 검증했으며 `AkashaColors.*`는 **328 lines / 81 files**를 유지한다.

UX-5D 검증: 해당 단계의 root analyze/test와 Windows Debug/Release build가 통과했다. `LibraryTheme`·legacy picker/preferences runtime type을 제거하고 `AkashaThemeRegistry` 불변식, effect group 독립성, Material interaction projection을 테스트로 고정했다. 기존 10개 Windows golden은 갱신 없이 통과했으며 `AkashaColors.*`는 **328 lines / 81 files**를 유지한다. 현재 registry는 bundled compile-time preset의 SSOT이며, 다운로드형 pack manifest·signature·cache loader는 별도 후속 계약이다. 현재 전체 품질 기준선은 [CURRENT_STATE.md](CURRENT_STATE.md)를 따른다.
...
| Theme migration 현실과 UX-0 inventory | [UX_THEME_MIGRATION_INVENTORY.md](../history/closure-2026-07/UX_THEME_MIGRATION_INVENTORY.md) |
| Theme package와 시각 회귀 matrix | [UX_THEME_REGRESSION_MATRIX.md](UX_THEME_REGRESSION_MATRIX.md) |

기존 R14 UI/UX 감사와 Home redesign 문서는 [history/closure-2026-07/ux-discovery/](../history/closure-2026-07/ux-discovery/README.md)에 보존된 역사적 분석 자료다. 현재의 UX 결정은 이 문서를 따른다.

...
| 표면 | 변경 |
|------|------|
| Fusion Search | Entity type badge · filter chips |
| Add dialog | Type picker (Work / Person / Concept / …) |
| Browse | **Filter:** Work \| Person \| Event \| All — grid **유지** |
| Workbench | non-work journal template |

### 7.1 Add flow (Person 예)

```
Fusion → 직접 추가 → Type: Person
  → pe_u_* catalog upsert
  → (선택) entities/person/{title}.md
```

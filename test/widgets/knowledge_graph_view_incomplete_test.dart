...
void main() {
  group('KnowledgeGraphView Incomplete Canvas UI Tests', () {
    ...
    testWidgets('renders empty state when complete 0 and incomplete 0', (tester) async {
    ...
    testWidgets('renders incomplete alert state when complete 0 and incomplete >= 1', (tester) async {
      // Create incomplete canvas (missing canvas.md)
      final incDir = Directory(p.join(tempVault.path, 'canvases', 'cv_u_broken'));
      await incDir.create(recursive: true);
      await File(p.join(incDir.path, 'layout.json')).writeAsString('{"canvas_id": "cv_u_broken"}');
    ...
    testWidgets('renders complete list and non-blocking warning when complete >= 1 and incomplete >= 1', (tester) async {
      ...
      final incDir = Directory(p.join(tempVault.path, 'canvases', 'cv_u_broken2'));
      await incDir.create(recursive: true);
      await File(p.join(incDir.path, 'layout.json')).writeAsString('{"canvas_id": "cv_u_broken2"}');

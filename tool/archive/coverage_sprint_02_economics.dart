...
/// Coverage Sprint 02 ??Registry-wide Coverage Economics (?¥ÏòÅ Í∑úÎ™® Ï∂îÏ†ï).
///
/// Usage: dart run tool/archive/coverage_sprint_02_economics.dart
///
/// ?∞Ï∂úÎ¨? akasha-db/pipeline/artifacts/coverage_dashboard/sprint_02_economics.json
...
/// Sprint 01 Î≥¥Ï†ï: ?ëÌíà??ÏµúÏÜå enrich( titles.en ¬∑ romaji ¬∑ alias ) Í≤Ä???¨Ìï® Î∂?const _minutesAutoHigh = 2.0; // tmdb/steam ??Î∞∞Ïπò fetch + spot check
const _minutesAutoMedium = 5.0; // Í∏∞Ì? externalId
const _minutesManualLow = 8.0; // partial titles ¬∑ ?ôÏùº franchise ?úÌîåÎ¶?const _minutesManualHigh = 15.0; // externalId ?ÜÏùå ¬∑ ?òÎèô Ï°∞ÏÇ¨
...
void main() {
  ...
  final report = {
    ...
    'method': {
      'purpose': 'Registry-wide Coverage 90% ?¥ÏòÅ ÎπÑÏö© Ï∂îÏ†ï ??Íµ¨Ï°∞ Í≤ÄÏ¶??ÑÎãò',
      'costUnit': 'maintainer_minutes per work (Sprint 01 Î≥¥Ï†ï)',
      'automationTier': {
        'auto_high': 'externalIds.tmdb|steam|igdb ??Í∏∞Ï°¥ ?ÑÍµ¨Î°?titles.en¬∑ja ?ÑÎ≥¥',
        'auto_medium': 'Í∏∞Ì? externalId ???åÏä§Î≥?Î∞òÏûê??,
        'manual_low': 'partial titles ?êÎäî franchise ?ïÏ†ú ?úÌîåÎ¶?Î≥µÏ†ú',
        'manual_high': 'externalId ?ÜÏùå ???òÎèô Ï°∞ÏÇ¨',
      },
      'sprint01Evidence':
          '17 works minimal enrich ??SW1/URV 81.6%??00% without structure change',
    },
  };
...
Map<String, dynamic> _sprint01Calibration() => {
      ...
      'note': 'Sprint 01 ?®Ïπò ?ëÏÑ±¬∑?ÅÏö©¬∑?åÍ? ?©ÏÇ∞ ~2.3h / 17????8Î∂???(Í≤Ä???¨Ìï®)',
    };
...
Map<String, dynamic> _compositeEstimate(Map<String, dynamic> registryWide) {
  ...
  return {
    ...
    'note':
        'Ï∂ïÎ≥Ñ ?ëÏóÖ Ï§ëÎ≥µ Í∞Ä????composite??titles.en 90% ?®Ïùº Ï∂?Î≥¥Ïàò Ï∂îÏ†ï',
  };
}
...
List<Map<String, dynamic>> _gapPanelCases() => [
      ...
      {'id': 'GAP-cjk-01', 'variant': 'È¨ºÊªÖ??àÉ', 'workIds': ['wk_000000343', 'wk_000000188']},
      {'id': 'GAP-cjk-02', 'variant': 'È¨ºÁÅ≠‰πãÂàÉ', 'workIds': ['wk_000000343', 'wk_000000188']},
      {'id': 'GAP-cjk-03', 'variant': 'Ê≠ª‰∫°Á¨îË?', 'workIds': ['wk_000000187']},
      {'id': 'GAP-cjk-04', 'variant': '?´ÂΩ±ÂøçËÄ?, 'workIds': ['wk_000000218']},
      {'id': 'GAP-alias-01', 'variant': 'Re:?º„É≠', 'workIds': ['wk_000000230']},

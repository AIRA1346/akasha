...
/// 402??k Scale Simulation ??SIM-A/B/C/D (Baseline v1 Í≤ÄÏ¶?.
///
/// Usage:
///   dart run tool/archive/scale_5k_sim.dart
///   dart run tool/archive/scale_5k_sim.dart --sim a
///   dart run tool/archive/scale_5k_sim.dart --sim a,b,c,d --seed 42
///
/// ?∞Ï∂úÎ¨?(gitignored Í∂åÏû•):
...
Future<Map<String, dynamic>> runSimA(Directory root, Random rng) async {
  ...
  return {
    ...
    'impact': verdict == 'FAIL'
        ? 'G1(5k) Í≥µÍ∏â Í≤ΩÎ°ú ÎØ∏ÏÑ±Î¶???Growth Strategy ?¨Í????Ä??(Î£®Ìä∏ Í∞Ä??Î∂ïÍ¥¥)'
        : verdict == 'PARTIAL'
            ? 'throughput ?êÎäî ?¥ÏòÅ Î∂Ä??Ï§??òÎÇòÎß?G1-1 Ï∂©Ï°± ???åÏù¥?ÑÎùº?∏¬∑Ïö¥??Ï°∞Ï†ï ?ÑÏöî'
            : 'G1-1 Í≤åÏù¥??Ï∂©Ï°± Í∞Ä?????§Ìôï???åÏùº??Ï∞©Ïàò ?ÑÏ†ú ?±Î¶Ω',
...
Map<String, dynamic> runSimB(Directory root, Random rng) {
  ...
  return {
    ...
    'impact': verdict == 'FAIL'
        ? 'Î∞∞Ïπò ?†ÏûÖ ??Ï§ëÎ≥µ wk_ ?§Ïóº ??ingest Í≤åÏù¥??Baseline Ï∂îÍ? ?ÑÏöî'
        : verdict == 'PARTIAL'
            ? '?¨ÌõÑ dedupe_linter??Î≥¥ÏôÑ Í∞Ä?•Ìïò??pre-insert recall Î∂ÄÏ°???shadow_write ?òÏ°¥ ?ÑÌóò'
            : '5k Íµ¨Í∞Ñ ?¨ÌõÑ dedupe + shadow_writeÎ°?A3 ?†Ï? Í∞Ä??,
...
Map<String, dynamic> runSimC(Directory root, Random rng) {
  ...
  return {
    ...
    'observations': {
      ...
      'tokenCollisionNote':
          '5k ?ïÏû• ??searchTokens Î∂ÄÎ∂ÑÏùºÏπ?stub??recall@10 ?úÏúÑÎ•?Î∞Ä?¥ÎÇº ???àÏùå',
    },
    'impact': verdict == 'FAIL'
        ? 'stub-first ?Ä???†ÏûÖ??SW1 recall ?Ä????enrich SLAÎ•?G1 ?ÑÏ†úÎ°??πÍ≤© ?ÑÏöî'
        : verdict == 'PARTIAL'
            ? 'enrich Í∑úÏú® ?ÜÏù¥ ?ïÏû• ???àÏßà Î¶¨Ïä§????G1-3 Ï°∞Í±¥Î∂Ä ?µÍ≥º'
            : 'enrich Î™©Ìëú(70%+) ?ÅÏö© ??5k?êÏÑú SW1 recall ?†Ï? Í∞Ä??,
    'baselineChangeRequired': verdict == 'FAIL',
    'growthStrategyReview': false,
  };
}
...
Map<String, dynamic> runSimD(Directory root, Random rng) {
  ...
  // maintainer 25% ?úÍ∞Ñ??franchise???†Îãπ (??5h)
  ...
  return {
    ...
    'observations': {
      ...
      'assumption':
          'ÏßÄ???ùÏÑ± ??2√ó2 ?åÍ∑úÎ™?defer + 60 IP ?†Í∑ú ?¨Î°ú?§Î??îÏñ¥ ??Ï£ºÏûÖ, ?úÏÑ± ?¥Îü¨?§ÌÑ∞??15Î∂?+ ÎØ∏Ïª§Î≤?Î©§Î≤Ñ 5Î∂?,
    },
    'impact': verdict == 'FAIL'
        ? '5k?êÏÑú franchise ?òÎèô ????∞ú ??ÏßÄ???ùÏÑ± ?ïÏ±Ö¬∑headcount ?ÑÏ†ú ?¨Í???
        : verdict == 'PARTIAL'
            ? 'ÏßÄ???ùÏÑ±?ºÎ°ú Î≤ÑÌã∏ ???àÏúº?????ÑÏ†Å ?úÏûë ??G1 Ï§?tier ?ïÏ±Ö ?òÏπò???ÑÏöî'
            : '5k Íµ¨Í∞Ñ franchise ?¥ÏòÅ ÎπÑÏö© ?µÏ†ú Í∞Ä??,
    'baselineChangeRequired': verdict == 'FAIL',
    'growthStrategyReview': false,
  };
}
...
List<Map<String, dynamic>> _syntheticWikidataNodes(
  ...
  for (var i = 0; i < count; i++) {
    ...
    nodes.add({
      ...
      'titles': {
        'en': 'Simu Manga $qid',
        if (rng.nextDouble() < 0.5) 'ja': '?∑„Éü?•Êº´??i',
      },
  ...
  for (var i = 0; i < count; i++) {
    ...
    drafts.add({
      ...
      'titles': {
        ...
        if (rng.nextDouble() < 0.3) 'ja': '?êÈï∑‰ΩúÂìÅ$seq',
      },
      'aliases': ['GW$seq'],
    });
  }
...
List<Map<String, dynamic>> _buildScaledIndex({
  ...
  while (out.length < targetCount) {
    ...
    final titles = <String, String>{
      if (hasEn) 'en': title,
      if (rng.nextDouble() < 0.25) 'ja': '?êÈï∑$seq',
      if (rng.nextDouble() < 0.10) 'ko': '?±Ïû• $seq',
    };
...
List<_GsQuery> _loadGsQueries(Directory root) {
  ...
  for (final line in lines) {
    ...
    final exclude = expectedRaw.contains('ÎØ∏ÏàòÎ°?) ||
        expectedRaw.contains('NOT_IN_REGISTRY') ||
        expectedRaw.trim() == '?? ||
        expectedRaw.contains('402??) && !expectedRaw.contains('wk_');
...
_FranchiseClusterStats _franchiseClusters(
  ...
  for (final members in clusters.values) {
    ...
    // ADR-006 ÏßÄ???ùÏÑ±: 2?ëÌíà¬∑2Îß§Ï≤¥ ?åÍ∑úÎ™??¥Îü¨?§ÌÑ∞??tier-2 defer
...
String _ipTitleStem(String title) {
  ...
  // Í≥µÌÜµ IP ?ëÎëê (Growth Stub 12345 ??growthstu)

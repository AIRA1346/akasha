...
Map<String, dynamic> runSimB(Directory root, Random rng) {
  ...
  return {
    ...
    'impact': verdict == 'FAIL'
        ? 'Î∞∞Ïπò ?†Ïû
 ??Ï§ëÎ≥µ wk_ ?§Ïóº ??ingest Í≤åÏù¥??Baseline Ï∂îÍ? ?ÑÏöî'
        : verdict == 'PARTIAL'
...
Map<String, dynamic> runSimC(Directory root, Random rng) {
  ...
  return {
    ...
    'impact': verdict == 'FAIL'
        ? 'stub-first ?Ä???†Ïû
??SW1 recall ?Ä????enrich SLAÎ•?G1 ?ÑÏ†úÎ°??πÍ≤© ?ÑÏöî'
        : verdict == 'PARTIAL'
...
Map<String, dynamic> runSimD(Directory root, Random rng) {
  ...
  return {
    ...
    'observations': {
      ...
      'assumption':
          'ÏßÄ???ùÏÑ± ??2√ó2 ?åÍ∑úÎ™?defer + 60 IP ?†Í∑ú ?¨Î°ú?§Î??îÏñ¥ ??Ï£ºÏû
, ?úÏÑ± ?¥Îü¨?§ÌÑ∞??15Î∂?+ ÎØ∏Ïª§Î≤?Î©§Î≤Ñ 5Î∂?,
    },
    'impact': verdict == 'FAIL'
        ? '5k?êÏÑú franchise ?òÎèô ????∞ú ??ÏßÄ???ùÏÑ± ?ïÏ±
¬∑headcount ?ÑÏ†ú ?¨Í???
        : verdict == 'PARTIAL'
            ? 'ÏßÄ???ùÏÑ±?ºÎ°ú Î≤ÑÌã∏ ???àÏúº?????ÑÏ†Å ?úÏûë ??G1 Ï§?tier ?ïÏ±
 ?òÏπò???ÑÏöî'
